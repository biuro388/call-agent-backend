export default async function handler(req, res) {
  // CORS - pozwala na wywołania z artefaktów Claude i przeglądarki
...
export default async function handler(req, res) {
    ...
    if (!VAPI_PRIVATE_KEY) {
          return res.status(500).json({ error: "Brak VAPI_PRIVATE_KEY w zmiennych srodowiskowych serwera." });
    }
    ...
    try {
          ...
          if (!vapiResponse.ok) {
                  return res.status(vapiResponse.status).json({
                            error: data.message || data.error || "Blad Vapi API",
                            details: data,
                  });
          }
